...
//uint8_t GPS_mode  = GPS_MODE_NONE;
// contains the current selected gps flight mode --> moved to the f. structure
...
#define DYN_THR_PID_CHANNEL THROTTLE
    ...
    // breakpoint is fix: 1500
  {
    ...
    {
      //  /512 instead of /500
      prop2 -= ((uint16_t)conf.dynThrPID*(rcData[DYN_THR_PID_CHANNEL]-1500)>>9);
    }
    else
    {
  ...
  switch (analogReader++ % (3+VBAT_CELLS_NUM))
  {
    case 0:
      {
        ...
        // [100mA]    //old (will overflow for 65A: powerValue * conf.pint2ma; // [1mA]
        analog.amperage = ((uint32_t)powerValue * conf.pint2ma) / 100;
        // [10 mA * msec]
    ...
    case 2:
      ...
      // here analogReader >=4, because of ++ in switch()
    default:
      {
#if defined(VBAT) && defined(VBAT_CELLS)
...
#if !(defined(SERIAL_RX) && defined(PROMINI))
  serialCom();
#endif
...
void setup()
{
...
#if GPS
  //Load GPS configuration parameteres
  recallGPSconf();
#endif
  ...
  // this speeds up analogRead without loosing too much resolution: http://www.arduino.cc/cgi-bin/yabb2/YaBB.pl?num=1208715493/11
  ...
  // #powercycle/reset/initialize events
  plog.start++;
...
void go_disarm()
{
  if (f.ARMED)
  {
    ...
    plog.disarm++;
    // lifetime in seconds
    plog.armed_time = armedTime;
    if (failsafeEvents)
    {
      // #acitve failsafe @ disarm
      plog.failsafe++;
    }
    ...
    // toggle @ arm & disarm to monitor for clean shutdown vs. powercut
    plog.running = 0;
    // write now.
    writePLog();
#endif
...
void loop ()
{
  ...
  if ((spekFrameDone == 0x01) || ((int16_t)(currentTime-rcTime) >0 ))
  {
      ...
      // not in rc loop
    {
      ...
      switch (taskOrder)
      {
        ...
        case 1:
          ...
          if (Baro_update() != 0)
          {
            // for MS baro: I2C set and get: 220 us  -  presure and temperature computation 160 us
            break;
          }
#endif
        case 2:
        ...
        case 3:
          ...
          if (GPS_Compute() != 0)
          {
            // performs computation on new frame only if present
            break;
          }
#if defined(I2C_GPS)
          if (GPS_NewData() != 0)
          {
            // 160 us with no new data / much more with new data
            break;
          }
#endif
#endif
    ...
    if (abs(rcCommand[YAW]) <70 && f.MAG_MODE)
    {
      ...
      {
        //Always correct maghold in GPS mode
    ...
    //check
    SerialWrite16(2,(nav[LAT]+rcCommand[PITCH])-(nav[LON]+rcCommand[ROLL]));
#endif
...
#define ACC_I_MAX 256
    ...
    for(axis=0;axis<3;axis++)
    {
        ...
        // MODE relying on ACC
      {
        ...
        //16 bits is ok here
        errorAngle = constrain((rcCommand[axis]<<1) + GPS_angle[axis],-500,+500) - att.angle[axis] + conf.angleTrim[axis];
      }
      if (axis == 2)
