...
//in+out message      to be used for a generic framework : MSP + function code (LIST/GET/SET) + data. no code yet
...
//out message         FRSKY Battery Cell Voltages
...
//in message          Sets nav config parameters - write to the eeprom
...
void serialCom()
{
  ...
  // limit max time in this function in case of GPS
  uint32_t timeMax;
...
#define RX_COND && (RX_SERIAL_PORT != port)
    ...
    if (GPS_SERIAL == port)
    {
      //Last gps frame seen at this time, used to detect stalled gps communication
      static uint32_t GPS_last_frame_seen;
      if (GPS_newFrame(c))
      {
    ...
    {
      // Limit the maximum execution time of serial decoding to avoid time spike
      return;
    }
#endif
    // SUPPRESS_ALL_SERIAL_MSP
...
void evaluateCommand(uint8_t c)
{
  ...
  switch(c)
  {
...
#ifdef LOG_PERMANENT
      misc.f = plog.arm;
      // <- computation must be moved outside from serial
      misc.g = plog.lifetime + (plog.armed_time / 1000000);
#else
      misc.f = 0; misc.g =0;
#endif
    ...
    case MSP_IDENT:
      ...
      //Navi version is stored in the upper four bits;
      id.cap   = (0+BIND_CAPABLE)|DYNBAL<<2|FLAP<<3|NAVCAP<<4|EXTAUX<<5|((uint32_t)NAVI_VERSION<<28);
    ...
    case MSP_SERVO_CONF:
      // struct servo_conf_ is 7 bytes length: min:2 / max:2 / middle:2 / rate:1    ----     8 servo =>  8x7 = 56
    ...
    case MSP_NAV_CONFIG:
      ...
      // to move to struct transmission
    case MSP_NAV_STATUS:
      headSerialReply(7);
    ...
    case MSP_SET_WP:
      {
          ...
          // The Nav state is not zero, so navigation is in progress, silently ignore SET_WP command)
        {
          ...
          //Not home and not poshold, we are free to store it in the eprom
          if (mission_step.number >0 && mission_step.number<255)
          {
            // Do not thrash the EEPROM with invalid wp number
            if (mission_step.number <= getMaxWPNumber())
            {
              storeWP();
            }
          }
