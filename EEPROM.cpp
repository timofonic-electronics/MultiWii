  ...
  if(calculate_sum((uint8_t*)&global_conf, sizeof(global_conf)) != global_conf.checksum)
    {
      ...
      // for config error signalization
  ...
  for(i=0;i<11;i++)
    {
      ...
      // [0;10000] -> [conf.minthrottle;MAXTHROTTLE]
      lookupThrottleRC[i] = conf.minthrottle + (uint32_t)((uint16_t)(MAXTHROTTLE-conf.minthrottle))* lookupThrottleRC[i]/10000;  
    }
#if defined(POWERMETER)
  // need to cast before multiplying
  pAlarm = (uint32_t) conf.powerTrigger1 * (uint32_t) PLEVELSCALE * (uint32_t) PLEVELDIV; 
#endif
#if GPS
  // at this time we don't have info about GPS init done
  GPS_set_pids();
  // Load gps parameters
  recallGPSconf(); 
#endif
...
#if GPS
  //Write GPS parameters
  writeGPSconf();
  //Read it to ensure correct eeprom content
  recallGPSconf(); 
#endif
...
void loadGPSdefaults(void)
{
  ...
  for (int i=0;i<sizeof(GPS_conf);i++)
    {
      *ptr++ = 0;
    }
