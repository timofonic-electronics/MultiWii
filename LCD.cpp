...
// ************************************************************************************************************
// Descriptors
static lcd_param_def_t __P =
  {
    &LTU8, 1, 1, 1
  };
static lcd_param_def_t __I =
  {
    &LTU8, 3, 1, 1
  };
static lcd_param_def_t __D =
  {
    &LTU8, 0, 1, 1
  };
static lcd_param_def_t __RC =
  {
    &LTU8, 2, 1, 1
  };
static lcd_param_def_t __PM =
  {
    &LPMM, 1, 1, 0
  };
/*static lcd_param_def_t __PS =
  {
  &LPMS, 1, 1, 0
  };
*/
static lcd_param_def_t __PT =
  {
    &LTU8, 0, 1, 1
  };
static lcd_param_def_t __VB =
  {
    &LTU8, 1, 1, 0
  };
static lcd_param_def_t __L =
  {
    &LTU8, 0, 1, 0
  };
static lcd_param_def_t __FS =
  {
    &LTU8, 1, 1, 0
  };
static lcd_param_def_t __SE =
  {
    &LTU16, 0, 1, 10
  };
static lcd_param_def_t __SE1 =
  {
    &LTU16, 0, 1, 1
  };
static lcd_param_def_t __ST =
  {
    &LTS16, 0, 1, 10
  };
static lcd_param_def_t __AUX1 =
  {
    &LAUX1, 0, 1, 1
  };
static lcd_param_def_t __AUX2 =
  {
    &LAUX2, 0, 1, 8
  };
static lcd_param_def_t __AUX3 =
  {
    &LAUX3, 0, 1, 64
  };
static lcd_param_def_t __AUX4 =
  {
    &LAUX4, 0, 1, 512
  };
static lcd_param_def_t __BITS =
  {
    &LTS8, 0, 1, 1
  };
  ...
  while (LCD == 1)
    {
      ...
      if (key == LCD_MENU_SAVE_EXIT || (IsLow(YAW) && IsHigh(PITCH) && allow_exit))
        ...
        {
          // exit without save: eeprom has only 100.000 write cycles
          LCD = 2;
        }
...
#define MAX_SERV PRI_SERVO_TO
  ...
  void output_mAh()
  {
    ...
    if (conf.powerTrigger1)
      {
        // bar graph powermeter (scale intPowerMeterSum/powerTrigger1 with *100/PLEVELSCALE)
...
#define POSSIBLE_OFFSET 0
  ...
  switch (telemetry)
    ...
    case '4':
      {
        static char channelNames[8][4] =
          {
            "Ail", "Ele", "Yaw", "Thr", "Ax1", "Ax2", "Ax3", "Ax4"
          };
        ...
        else
          {
            ...
            if ((PRI_SERVO_FROM <= j && PRI_SERVO_TO >= j) || (SEC_SERVO_FROM <= j && SEC_SERVO_TO >= j))
              ...
              if (j < PRI_SERVO_FROM)
                ...
                {
                  // (PRI_SERVO_FROM <= j && PRI_SERVO_TO >= j)
                }
#endif
#if defined(PRI_SERVO_FROM) || defined(SEC_SERVO_FROM)
            {
