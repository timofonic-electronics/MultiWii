...
#if defined(TEENSY20)
//for a quad+: rear,right,left,front
uint8_t PWM_PIN[8] = {14,15,9,12,22,18,16,17};
#elif defined(A32U4_4_HW_PWM_SERVOS)
uint8_t PWM_PIN[8] = {6,9,10,11,5,13,SW_PWM_P3,SW_PWM_P4};
#else
//for a quad+: rear,right,left,front
uint8_t PWM_PIN[8] = {9,10,5,6,4,A2,SW_PWM_P3,SW_PWM_P4};
#endif
#else
#if defined(TEENSY20)
//for a quad+: rear,right,left,front
uint8_t PWM_PIN[8] = {14,15,9,12,4,10,16,17};
#elif defined(A32U4_4_HW_PWM_SERVOS)
uint8_t PWM_PIN[8] = {6,9,10,11,5,13,SW_PWM_P3,SW_PWM_P4};
#else
//for a quad+: rear,right,left,front
uint8_t PWM_PIN[8] = {9,10,5,6,11,13,SW_PWM_P3,SW_PWM_P4};
#endif
#endif
#endif
#if defined(MEGA)
//for a quad+: rear,right,left,front   //+ for y6: 7:under right  8:under left
...
volatile uint8_t atomicServo[8] = {
  125,
  125,
  125,
  125,
...
#if defined(AIRPLANE)|| defined(HELICOPTER)
// To prevent motor to start at reset. atomicServo[7]=5 or 249 if reversed servo
volatile uint16_t atomicServo[8] = {
  8000,
  8000,
  8000,
  8000,
...
#if defined(PRI_SERVO_FROM) && !defined(HW_PWM_SERVOS)
  ...
  // [1000;2000] => [125;250]
{
...
#ifndef EXT_MOTOR_RANGE
  //  pin 6
  TC4H = motor[0]>>8; OCR4D = (motor[0]&0xFF);
#else
  //  pin 6
  TC4H = (((motor[0]-1000)<<1)+16)>>8; OCR4D = ((((motor[0]-1000)<<1)+16)&0xFF);
#endif
#else
...
#ifndef EXT_MOTOR_RANGE
  //  pin 5
  TC4H = (2047-motor[2])>>8; OCR4A = ((2047-motor[2])&0xFF);
#else
  //  pin 5
  TC4H = 2047-(((motor[2]-1000)<<1)+16)>>8; OCR4A = (2047-(((motor[2]-1000)<<1)+16)&0xFF);
#endif
#else
#ifdef EXT_MOTOR_RANGE
...
#ifdef EXT_MOTOR_RANGE
  //  pin 6
  TC4H = (((motor[3]-1000)<<1)+16)>>8; OCR4D = ((((motor[3]-1000)<<1)+16)&0xFF);
#elif defined(EXT_MOTOR_64KHZ)
  Temp2 = Temp2 >> 2;
  TC4H = Temp2 >> 8;
  //  pin 6
...
#else
  //  pin 6
  TC4H = motor[3]>>8; OCR4D = (motor[3]&0xFF);
#endif
#endif
#if (NUMBER_MOTOR > 4)
#if !defined(HWPWM6)
  ...
  OCR1C = motor[4]<<3;
  //  pin 13
  TC4H = motor[5]>>8; OCR4A = (motor[5]&0xFF);
#else
  OCR1C = ((motor[4]<<4) - 16000) + 128;
  //  pin 13
  TC4H = (((motor[5]-1000)<<1)+16)>>8; OCR4A = ((((motor[5]-1000)<<1)+16)&0xFF);
#endif
#endif
#endif
#if (NUMBER_MOTOR > 6)
  ...
  //note: EXT_MOTOR_RANGE not possible here
#else
...
void initOutput()
{
...
#endif
#if (NUMBER_MOTOR > 1)
  // connect pin 5 to timer 3 channel A
  TCCR3A |= _BV(COM3A1);
#endif
#if (NUMBER_MOTOR > 2)
  // init 16bit timer 4
  ...
  // connect pin 6 to timer 4 channel A
  TCCR4A |= _BV(COM4A1);
#endif
#if (NUMBER_MOTOR > 3)
...
#endif
#if (NUMBER_MOTOR > 4)
  // connect pin 7 to timer 4 channel B
  TCCR4A |= _BV(COM4B1);
  // connect pin 8 to timer 4 channel C
  TCCR4A |= _BV(COM4C1);
#endif
#if (NUMBER_MOTOR > 6)
  // timer 2 is a 8bit timer so we cant change its range
  // connect pin 9 to timer 2 channel B
  TCCR2A |= _BV(COM2B1);
  // connect pin 10 to timer 2 channel A
  TCCR2A |= _BV(COM2A1);
#endif
#endif
  ...
  ICR3   = 0x00FF;
  TC4H = 0x00;
  // phase and frequency correct mode & top to 255
  OCR4C = 0xFF;
  // prescaler to 1
  TCCR4B = (1<<CS40);
#elif defined(EXT_MOTOR_32KHZ)
  ...
  ICR3   = 0x01FF;
  TC4H = 0x01;
  // phase and frequency correct mode & top to 511
  OCR4C = 0xFF;
  // prescaler to 1
  TCCR4B = (1<<CS40);
#elif defined(EXT_MOTOR_16KHZ)
  ...
  TCCR4B = (1<<CS41);
#endif
  // connect pin 9 to timer 1 channel A
  TCCR1A |= _BV(COM1A1);
  // connect pin 10 to timer 1 channel B
  TCCR1A |= _BV(COM1B1);
  // connect pin 5 to timer 3 channel A
  TCCR3A |= _BV(COM3A1);
  TCCR4D = 0;
  ...
  // phase correct mode & no prescaler
  TCCR1A |= (1<<WGM11);
  TCCR1A &= ~(1<<WGM10);
  ...
  // phase and frequency correct mode & top to 1023 but with enhanced pwm mode we have 2047
  TCCR4D |= (1<<WGM40); TC4H = 0x3; OCR4C = 0xFF;
  // connect pin 5 to timer 4 channel A
...
#if (NUMBER_MOTOR > 4)
#if defined(HWPWM6)
  // connect pin 11 to timer 1 channel C
  TCCR1A |= _BV(COM1C1);
  // connect pin 13 to timer 4 channel A
...
#endif
#if (NUMBER_MOTOR > 2)
  // connect pin 11 to timer 2 channel A
  TCCR2A |= _BV(COM2A1);
#endif
#if (NUMBER_MOTOR > 3)
  // connect pin 3 to timer 2 channel B
  TCCR2A |= _BV(COM2B1);
#endif
  // PIN 5 & 6 or A0 & A1
#if (NUMBER_MOTOR > 4)
  ...
  // we reactivate the INPUT affectation for these two PINs
  pinMode(5,INPUT);pinMode(6,INPUT);
...
#endif
  // uses timer 3 Comperator A (11 bit)
#if (defined(PROMICRO) && !defined(HWPWM6))
  //normal counting & no prescaler
...
#define SERVO_TOP_VAL (uint16_t)(1000000L / SERVO_RFR_RATE)
  ...
  // phase correct mode & prescaler to 8 = 1us resolution
  TCCR5A |= (1<<WGM51);
  TCCR5A &= ~(1<<WGM50);
  ...
  // phase correct mode & prescaler to 8
  TCCR1A |= (1<<WGM11);
  TCCR1A &= ~(1<<WGM10);
...
#if (PRI_SERVO_FROM <= 8 && PRI_SERVO_TO >= 8) || (SEC_SERVO_FROM <= 8 && SEC_SERVO_TO >= 8)
#if defined(AIRPLANE) || defined(HELICOPTER)
  // Trhottle at minimum for airplane and heli
  servo[7] =  MINCOMMAND;
  OCR4C = MINCOMMAND;
#endif
  pinMode(8,OUTPUT);
...
// Fill in with variable to function. ex.) L1130
#define SERVO_PULSE(PIN_HIGH, ACT_STATE, SERVO_NUM, LAST_PIN_LOW) \
  ...
  ISR(SERVO_ISR)
  {
    // indicates the current state of the chain
    static uint8_t state = 0;
    if(state == 0)
    {
      // set servo 1's pin high
      SERVO_1_HIGH;
      // wait 1000us
      SERVO_CHANNEL+=SERVO_1K_US;
      // count up the state
    ...
    {
      // load the servo's value (0-1000us)
    ...
    else
    {
      ...
      // if there are 3 or more servos we dont need to slow it down
#if defined(SERVO_3_HIGH)
      // 0 would be better but it causes bad jitter
      SERVO_CHANNEL+=(SERVO_1K_US>>3);
      state=0;
      // if there are less then 3 servos we need to slow it to not go over 300Hz (the highest working refresh rate for the digital servos for what i know..)
#else
      SERVO_CHANNEL+=SERVO_1K_US;
      if(state<4)
      {
      ...
      // if there are 4 or more servos we dont need to slow it down
#if defined(SERVO_4_HIGH)
      // 0 would be better but it causes bad jitter
      SERVO_CHANNEL+=(SERVO_1K_US>>3);
      state=0;
      // if there are less then 4 servos we need to slow it to not go over ~170Hz (the highest working refresh rate for analog servos)
#else
      SERVO_CHANNEL+=SERVO_1K_US;
      if(state<8)
      {
...
#include MY_PRIVATE_MIXING
#elif defined( BI )
    //LEFT
    motor[0] = PIDMIX(+1, 0, 0);
    //RIGHT
    motor[1] = PIDMIX(-1, 0, 0);
    //LEFT
    servo[4] = (SERVODIR(4,2) * axisPID[YAW]) + (SERVODIR(4,1) * axisPID[PITCH]) + get_middle(4);
    //RIGHT
    servo[5] = (SERVODIR(5,2) * axisPID[YAW]) + (SERVODIR(5,1) * axisPID[PITCH]) + get_middle(5);
#elif defined( TRI )
    //REAR
    motor[0] = PIDMIX( 0,+4/3, 0);
    //RIGHT
    motor[1] = PIDMIX(-1,-2/3, 0);
    //LEFT
    motor[2] = PIDMIX(+1,-2/3, 0);
    //REAR
    servo[5] = (SERVODIR(5, 1) * axisPID[YAW]) + get_middle(5);
#elif defined( QUADP )
    //REAR
    motor[0] = PIDMIX( 0,+1,-1);
    //RIGHT
    motor[1] = PIDMIX(-1, 0,+1);
    //LEFT
    motor[2] = PIDMIX(+1, 0,+1);
    //FRONT
    motor[3] = PIDMIX( 0,-1,-1);
#elif defined( QUADX )
    //REAR_R
    ...
    //REAR_2 CCW
    motor[2] = PIDMIX(+0,+1,+1);
    //FRONT_L CW
    motor[3] = PIDMIX(+1,-1, 0);
#elif defined( Y6 )
    //REAR
    motor[0] = PIDMIX(+0,+4/3,+1);
    //RIGHT
    motor[1] = PIDMIX(-1,-2/3,-1);
    //LEFT
    motor[2] = PIDMIX(+1,-2/3,-1);
    //UNDER_REAR
    motor[3] = PIDMIX(+0,+4/3,-1);
    //UNDER_RIGHT
    motor[4] = PIDMIX(-1,-2/3,+1);
    //UNDER_LEFT
    motor[5] = PIDMIX(+1,-2/3,+1);
#elif defined( HEX6 )
    //REAR_R
    motor[0] = PIDMIX(-7/8,+1/2,+1);
    //FRONT_R
    motor[1] = PIDMIX(-7/8,-1/2,-1);
    //REAR_L
    motor[2] = PIDMIX(+7/8,+1/2,+1);
    //FRONT_L
    motor[3] = PIDMIX(+7/8,-1/2,-1);
    //FRONT
    motor[4] = PIDMIX(+0  ,-1  ,+1);
    //REAR
    motor[5] = PIDMIX(+0  ,+1  ,-1);
#elif defined( HEX6X )
    //REAR_R
    motor[0] = PIDMIX(-1/2,+7/8,+1);
    //FRONT_R
    motor[1] = PIDMIX(-1/2,-7/8,+1);
    //REAR_L
    motor[2] = PIDMIX(+1/2,+7/8,-1);
    //FRONT_L
    motor[3] = PIDMIX(+1/2,-7/8,-1);
    //RIGHT
    motor[4] = PIDMIX(-1  ,+0  ,-1);
    //LEFT
    motor[5] = PIDMIX(+1  ,+0  ,+1);
#elif defined( HEX6H )
    //REAR_R
    motor[0] = PIDMIX(-1,+1,-1);
    //FRONT_R
    ...
    motor[3] = PIDMIX(+ 1,-1,-1);
    //RIGHT
    motor[4] = PIDMIX(0 ,0 ,0);
    //LEFT
    motor[5] = PIDMIX(0 ,0 ,0);
#elif defined( OCTOX8 )
    //REAR_R
    motor[0] = PIDMIX(-1,+1,-1);
    //FRONT_R
    ...
    //UNDER_REAR_L
    motor[6] = PIDMIX(+1,+1,-1);
    //UNDER_FRONT_L
    motor[7] = PIDMIX(+1,-1,+1);
#elif defined( OCTOFLATP )
    //FRONT_L
    motor[0] = PIDMIX(+7/10,-7/10,+1);
    //FRONT_R
    motor[1] = PIDMIX(-7/10,-7/10,+1);
    //REAR_R
    motor[2] = PIDMIX(-7/10,+7/10,+1);
    //REAR_L
    motor[3] = PIDMIX(+7/10,+7/10,+1);
    //FRONT
    motor[4] = PIDMIX(+0, -1, -1);
    //RIGHT
    motor[5] = PIDMIX(-1   ,+0   ,-1);
    //REAR
    motor[6] = PIDMIX(+0   ,+1   ,-1);
    //LEFT
    motor[7] = PIDMIX(+1   ,+0   ,-1);
#elif defined( OCTOFLATX )
    //MIDFRONT_L
    motor[0] = PIDMIX(+1  ,-1/2,+1);
    //FRONT_R
    motor[1] = PIDMIX(-1/2,-1  ,+1);
    //MIDREAR_R
    motor[2] = PIDMIX(-1  ,+1/2,+1);
    //REAR_L
    motor[3] = PIDMIX(+1/2,+1  ,+1);
    //FRONT_L
    motor[4] = PIDMIX(+1/2,-1  ,-1);
    //MIDFRONT_R
    motor[5] = PIDMIX(-1  ,-1/2,-1);
    //REAR_R
    motor[6] = PIDMIX(-1/2,+1  ,-1);
    //MIDREAR_L
    motor[7] = PIDMIX(+1  ,+1/2,-1);
#elif defined( VTAIL4 )
    //REAR_R
    motor[0] = PIDMIX(+0,+1, +1);
    //FRONT_R
    ...
    {
      //   Wing 1
      servo[3] = rcCommand[ROLL] + flapperons[0];
      //   Wing 2
      servo[4] = rcCommand[ROLL] + flapperons[1];
      //   Rudder
      servo[5] = rcCommand[YAW];
      //   Elevator
    ...
    else
    {
      ...
      //   Wing 1
      servo[3] = axisPID[ROLL] + flapperons[0];
      //   Wing 2
      servo[4] = axisPID[ROLL] + flapperons[1];
      //   Rudder
      servo[5] = axisPID[YAW];
      //   Elevator
    ...
    {
      // servo rates
      servo[i]  = ((int32_t)conf.servoConf[i].rate * servo[i])/100L;
    ...
    for(i=3;i<7;i++)
    {
      // mix and setup direction
      servo[i]  = (axisPID[YAW] * SERVODIR(i,2)) + (axisPID[(6-i)>>1] * SERVODIR(i,1));
      servo[i] += get_middle(i);
    }
    ...
    {
      // mix and setup direction
      servo[i] =  axisPID[5-i] * SERVODIR(i,1);
      servo[i] += get_middle(i);
    }
    //  Pin D9
    motor[0] = PIDMIX(0,0,-1);
    //  Pin D10
    motor[1] = PIDMIX(0,0,+1);
    ...
    {
      // attack
      governorThrottle += delta_collective * conf.governorP;
    ...
    else
    {
      ...
      if (! (++d % conf.governorD))
      {
        // decay; signal stepsize 10 should  be smooth on most ESCs
        governorThrottle -= 10;
      }
    }
    if (governorThrottle < 0)
    {
      // always beware of negative impact of governor on throttle
      governorThrottle = 0;
    }
#endif
    ...
    // abs collective minus deadband
    int16_t acmd = abs(collective) - conf.yawCollPrecompDeadband;
    if (acmd > 0 )
    {
...
#endif
      //  limit min & max
      servo[7] = constrain(servo[7], conf.minthrottle, MAXTHROTTLE);
    }
#ifndef HELI_USE_SERVO_FOR_THROTTLE
    // use real motor output - ESC capable
    motor[0] = servo[7];
#if YAWMOTOR
    // use motor2 output for YAWMOTOR
    motor[1] = servo[5];
#endif
#endif
...
#define HeliXPIDMIX(Z,Y,X) ( (collRange[1] + collective)*Z + heliNick*Y +  heliRoll*X)/10
    ...
    //    NICK  servo
    servo[3]  =  HeliXPIDMIX( ( SERVODIR(3,4) * nickMix[0]), SERVODIR(3,2) * nickMix[1], SERVODIR(3,1) * nickMix[2]);
    //    LEFT  servo
    servo[4]  =  HeliXPIDMIX( ( SERVODIR(4,4) * leftMix[0]), SERVODIR(4,2) * leftMix[1], SERVODIR(4,1) * leftMix[2]);
    //    RIGHT servo
    servo[6]  =  HeliXPIDMIX( ( SERVODIR(6,4) * rightMix[0]),SERVODIR(6,2) * rightMix[1],SERVODIR(6,1) * rightMix[2]);
#endif
    /************************************************************************************************************/
#ifdef HELI_90_DEG
    //     NICK  servo
    servo[3]  = HeliXPIDMIX( +0, (conf.servoConf[3].rate/10), -0);
    //     ROLL servo
    servo[4]  = HeliXPIDMIX( +0, +0, (conf.servoConf[4].rate/10));
    //     COLLECTIVE  servo
...
#define CAM_TIME_LOW  conf.servoConf[2].middle
    ...
    for(i=SERVO_START-1; i<SERVO_END; i++)
    {
      if(i < 2)
      {
        // servo travel scaling, only for gimbal servos
        servo[i] = map(servo[i], 1020,2000, conf.servoConf[i].min, conf.servoConf[i].max);
      }
    ...
    // copy CamPitch value to propper output servo for A0_A1_PIN_HEX
    servo[3] = servo[0];
    // copy CamRoll  value to propper output servo for A0_A1_PIN_HEX
    servo[4] = servo[1];
#endif
#if defined(TRI) && defined(MEGA_HW_PWM_SERVOS) && defined(MEGA)
    // servo[5] is still use by gui for this config (more genereic)
    servo[5] = constrain(servo[5], conf.servoConf[5].min, conf.servoConf[5].max);
    // copy TRI serwo value to propper output servo for MEGA_HW_PWM_SERVOS
    servo[3] = servo[5];
#endif
#endif
...
#error "VOLTAGEDROP_COMPENSATION requires correction values which fit VBATNOMINAL; not yet defined for your value of VBATNOMINAL"
...
#if (LOG_VALUES >= 3) || defined(POWERMETER_SOFT)
    {
      ...
      uint16_t amp;
      // pseudo ampere * microseconds
      uint32_t ampsum, ampus;
      /* true cubic function;
        ...
        // by all means - must avoid division by zero
      {
        ...
        for (i =0;i<NUMBER_MOTOR;i++)
        {
          // range mapped from [1000:2000] => [0:1000]; then break that up into 64 ranges; lookup amp
          amp = amperes[ ((motor[i] - 1000)>>4) ] / analog.vbat;
          ampus = ( (currentTime-lastRead) * (uint32_t)amp * (uint32_t)conf.pint2ma ) / PLEVELDIVSOFT;
#if (LOG_VALUES >= 3)
          // sum up over time the mapped ESC input
          pMeter[i]+= ampus;
#endif
#if defined(POWERMETER_SOFT)
          // total sum over all motors
          ampsum += ampus;
#endif
        }
#if defined(POWERMETER_SOFT)
        // total sum over all motors
        pMeter[PMOTOR_SUM]+= ampsum / NUMBER_MOTOR;
#endif
      }
